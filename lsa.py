...
def makeTFIDF(docs=ENGLISHTRAINING, threshold=0, N=False):
    ...
    for tweet in codecs.open(docs, encoding="UTF-8").read().split("\n"):
        ...
        if not cols:
            cols = tweet.split()[2:]
            continue
        tweets.append(makeTweet(tweet.strip().split("\t"), idf))
    """
...
def savedict(classifier, out=sys.stdout):
    if isinstance(out, str):
        out = open(out, "w")
    json.dump([classifier.cols, classifier.scoredict], out)
...
def codaformat(testset, sentidict, out=sys.stdout, threshold=null, singleColumn=None):
    if threshold == null:
